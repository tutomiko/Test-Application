...
def get_ipv4_address(domain: str) -> str | None:
    ...
    try:
        ...
        if addr_info:
            ...
            # This case should ideally be covered by socket.gaierror if no results are found,
        ...
        print(f"An unexpected error occurred during IPv4 resolution: {e}", file=sys.stderr)
        return None
...
def main():
    ...
    if ipv4_address:
        ...
        # If get_ipv4_address returned None, we need to differentiate between
        # a completely unresolvable domain and a domain that only has IPv6.
        try:
            # Attempt to resolve for IPv6 to see if it exists
